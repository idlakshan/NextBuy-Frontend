...
const cartProductSlice = createSlice({
  ...
  reducers: {
    handleAddItemCart: (state, action) => {
      ...
      const summary = action.payload.reduce((acc, curr) => {
        ...
        return {
          ...
          totalPrice: acc.totalPrice + (priceAfterDiscount * (curr.quantity || 0)),
          notDiscountTotalPrice: acc.notDiscountTotalPrice + (curr?.productId?.price * (curr.quantity || 0)),
          totalDiscount: acc.totalDiscount + (
            (curr?.productId?.price - priceAfterDiscount) * (curr.quantity || 0)
          )
        };
      }, { totalQty: 0, totalPrice: 0, notDiscountTotalPrice: 0, totalDiscount: 0 });
      
      state.summary = summary;
    },
    clearCart: (state) => {
      state.cart = [];
      state.summary = { totalQty: 0, totalPrice: 0, notDiscountTotalPrice: 0, totalDiscount: 0 };
    }
  },
